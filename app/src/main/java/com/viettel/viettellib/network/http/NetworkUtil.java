...
import com.viettel.viettellib.network.rsa.AESUtil;
import com.viettel.viettellib.network.rsa.RSACryptoDer;
import com.viettel.viettellib.network.rsa.RSAUtil;
 ...
 * 
 * @author DungNTM
 * @since May 5, 2010 Copyright KunKun.Vn �2010 - All rights reserved. VIETTEL
...
public class NetworkUtil {
	...
	public static Bitmap getImage(String url) {
		...
		try {
			...
			// stream to DataInputStream and called readFully() instead of
			// read()
			...
			try {
				...
				if (imageData.length > 0) {
					...
					// <HaiTC
					im = BitmapFactory.decodeByteArray(imageData, 0,
							imageData.length);
	 ...
	 * @param method
	 * @param dt
	 *            : vector of actions and its param
	 * @return
	 * @Author : DoanDM Since Dec 6, 2010
	 */
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static String getJSONStringRSA(String method, Vector dt,
			ActionEvent actionEvent, boolean usingAES) {
		String org = "";
		try {
			...
			if (usingAES) {
				...
				orgObj.put(IntentConstants.INTENT_AES_KEY, aesKey);
				actionEvent.userData = aesKey;
			}
			for (int i = 0, size = dt.size(); i < size; i += 2) {
				orgObj.put(dt.elementAt(i).toString(), dt.elementAt(i + 1)
						.toString());
			}

			org = RSAUtil.getInstance().rsaEncrypt(orgObj.toString());

			org = org.replace("\r", "");
			org = org.replace("\n", "");

			JSONObject params = new JSONObject();
			params.put("request", org);
			...
			org = data.toString();
			Mylog.i("-----------------", "-----------");
			Mylog.i("REQUEST RSA:" + method, org);
			Mylog.i("-----------------", "-----------");
		} catch (Exception ex) {
	...
	public static String getJSONStringRSAJsonoject(JSONObject orgObj, ActionEvent actionEvent) {
		String org = "";
		try {
			...
			InputStream publickey = GlobalInfo.getInstance().getAppContext().getAssets().open("public.der");
			org=RSACryptoDer.encrypt(orgObj.toString().getBytes() ,RSACryptoDer.loadPublicKey_X509_RSA(publickey));	
//		org = RSAUtil.getInstance().rsaEncrypt(orgObj.toString());
			...
			JSONObject params = new JSONObject();
			params.put("request", org);
			params.put("aesKey", actionEvent.userData);
	...
	public static byte[] getMultiPartByte(String method, Vector dt,
	...
	// use for register
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static byte[] getMultiPartByte(String dataText, byte[] fileBytes,
			String fileName, String fileField, String fileType) throws Exception {
		byte[] postBytes = null;
		try {
			Hashtable params = new Hashtable();
			...
			System.out.println("Loi gui file: "+fileName);
			throw(ex);
		}
		return postBytes;
	}
	...
	@SuppressWarnings("rawtypes")
	static String getBoundaryMessage(String boundary, Hashtable params,
