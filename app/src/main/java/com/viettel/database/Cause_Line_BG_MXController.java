...
public class Cause_Line_BG_MXController {
	private Context mContext = null;
	public static final String[] allColumn = new String[] {
			Cause_Line_BG_MXField.COLUMN_CAUSE_LINE_BG_MX_ID,
			Cause_Line_BG_MXField.COLUMN_SUPERVISION_LINE_BG_MX_ID,
			Cause_Line_BG_MXField.COLUMN_CAT_CAUSE_CONSTR_UNQUALIFY_ID,
			Cause_Line_BG_MXField.COLUMN_SYNC_STATUS,
			Cause_Line_BG_MXField.COLUMN_IS_ACTIVE,
			Cause_Line_BG_MXField.COLUMN_PROCESS_ID,
			Cause_Line_BG_MXField.COLUMN_EMPLOYEE_ID,
			Cause_Line_BG_MXField.COLUMN_SUPERVISION_CONSTR_ID};

	public static final String CREATE_CAUSE_LINE_BG_MX_TABLE = "CREATE TABLE IF NOT EXISTS "
			...
			+ Cause_Line_BG_MXField.COLUMN_PROCESS_ID
			+ " INTEGER,"
			+ Cause_Line_BG_MXField.COLUMN_EMPLOYEE_ID + " TEXT," 
			+ Cause_Line_BG_MXField.COLUMN_SUPERVISION_CONSTR_ID
			+ " TEXT)";

	public Cause_Line_BG_MXController(Context pContext) {
		this.mContext = pContext;
	}

	public boolean addItem(Cause_Line_BG_MXEntity addItem) {
		Long idItem = 0L;
		SQLiteDatabase db = KttsDatabaseHelper.INSTANCE.open(mContext);
		ContentValues values = new ContentValues();
		values.put(Cause_Line_BG_MXField.COLUMN_SUPERVISION_CONSTR_ID,
				addItem.getSupervisionConstrId());
		
		values.put(Cause_Line_BG_MXField.COLUMN_CAUSE_LINE_BG_MX_ID,
				addItem.getCause_Line_Bg_Mx_Id());

		values.put(Cause_Line_BG_MXField.COLUMN_SUPERVISION_LINE_BG_MX_ID,
				addItem.getSupervision_Line_Bg_MX_Id());

		values.put(Cause_Line_BG_MXField.COLUMN_CAT_CAUSE_CONSTR_UNQUALIFY_ID,
				addItem.getCat_Cause_Constr_Unqualify_Id());

		values.put(Cause_Line_BG_MXField.COLUMN_SYNC_STATUS,
				addItem.getSync_Status());

		values.put(Cause_Line_BG_MXField.COLUMN_IS_ACTIVE,
				addItem.getIsActive());

		values.put(Cause_Line_BG_MXField.COLUMN_PROCESS_ID,
				addItem.getProcessId());

		values.put(Cause_Line_BG_MXField.COLUMN_EMPLOYEE_ID,
				addItem.getEmployeeId());
		// Inserting Row
		idItem = db.insert(Cause_Line_BG_MXField.TABLE_NAME, null, values);
	...
	public boolean delete(long idItem, int iStatus) {
		...
		String sqlclause = Cause_Line_BG_MXField.COLUMN_CAUSE_LINE_BG_MX_ID
				+ "= ?";
		String[] sqlPara = new String[] { String.valueOf(idItem) };
		return this.updateDB(Cause_Line_BG_MXField.TABLE_NAME, values,
				sqlclause, sqlPara);
	}
	...
	public List<Supervision_LBG_UnqualifyItemEntity> getAllMxUnqulifyByMxId(
		...
		Cursor cursor = db.query(
				...
				allColumn,
				Cause_Line_BG_MXField.COLUMN_SUPERVISION_LINE_BG_MX_ID
						+ " = ? AND " + Cause_Line_BG_MXField.COLUMN_IS_ACTIVE
						+ " = ?",
	...
	private Supervision_LBG_UnqualifyItemEntity converUnqualifyToUnqualifyItem(
		...
		try {
			curItem.setCause_Line_Bg_Id(cursor.getLong(cursor
					.getColumnIndex(Cause_Line_BG_MXField.COLUMN_CAUSE_LINE_BG_MX_ID)));

			curItem.setCat_Cause_Constr_Unqualify_Id(cursor.getLong(cursor
					.getColumnIndex(Cause_Line_BG_MXField.COLUMN_CAT_CAUSE_CONSTR_UNQUALIFY_ID)));
