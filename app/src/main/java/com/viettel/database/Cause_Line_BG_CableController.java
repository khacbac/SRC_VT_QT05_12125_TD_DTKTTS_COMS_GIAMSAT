...
public class Cause_Line_BG_CableController {
	public static final String[] allColumn = new String[] {
			Cause_Line_BG_CableField.COLUMN_CAUSE_LINE_BG_CABLE_ID,
			Cause_Line_BG_CableField.COLUMN_SUPERVISION_LINE_BG_CABLE_ID,
			Cause_Line_BG_CableField.COLUMN_CAT_CAUSE_CONSTR_UNQUALIFY_ID,
			Cause_Line_BG_CableField.COLUMN_SYNC_STATUS,
			Cause_Line_BG_CableField.COLUMN_IS_ACTIVE,
			Cause_Line_BG_CableField.COLUMN_PROCESS_ID,
			Cause_Line_BG_CableField.COLUMN_EMPLOYEE_ID,
			Cause_Line_BG_CableField.COLUMN_SUPERVISION_CONSTR_ID };

	private Context mContext = null;
	public static final String CREATE_CAUSE_LINE_BG_CABLE_TABLE = "CREATE TABLE IF NOT EXISTS "
	...
	public Cause_Line_BG_CableController(Context pContext) {
		this.mContext = pContext;
	}

	public boolean addItem(Cause_Line_BG_CableEntity addItem) {
		Long idItem = 0L;
		SQLiteDatabase db = KttsDatabaseHelper.INSTANCE.open(mContext);
		ContentValues values = new ContentValues();

		values.put(Cause_Line_BG_CableField.COLUMN_SUPERVISION_CONSTR_ID,
				addItem.getSupervisionConstrId());
		
		values.put(Cause_Line_BG_CableField.COLUMN_CAUSE_LINE_BG_CABLE_ID,
				addItem.getCause_Line_Bg_Cable_Id());
		values.put(
				Cause_Line_BG_CableField.COLUMN_SUPERVISION_LINE_BG_CABLE_ID,
		...
		values.put(Cause_Line_BG_CableField.COLUMN_SYNC_STATUS,
				addItem.getSync_Status());

		values.put(Cause_Line_BG_CableField.COLUMN_IS_ACTIVE,
				addItem.getIsActive());

		values.put(Cause_Line_BG_CableField.COLUMN_PROCESS_ID,
				addItem.getProcessId());

		values.put(Cause_Line_BG_CableField.COLUMN_EMPLOYEE_ID,
				addItem.getEmployeeId());

		// Inserting Row
		idItem = db.insert(Cause_Line_BG_CableField.TABLE_NAME, null, values);
	...
	public boolean delete(Supervision_LBG_UnqualifyItemEntity pDelItem) {
		...
		String sqlclause = Cause_Line_BG_CableField.COLUMN_CAUSE_LINE_BG_CABLE_ID
				+ "= ?";
		String[] sqlPara = new String[] { String.valueOf(pDelItem
				.getCause_Line_Bg_Id()) };
		return this.updateDB(Cause_Line_BG_CableField.TABLE_NAME, values,
				sqlclause, sqlPara);
	}
	...
	private Supervision_LBG_UnqualifyItemEntity converUnqualifyToUnqualifyItem(
		...
		try {
			curItem.setCause_Line_Bg_Id(cursor.getLong(cursor
					.getColumnIndex(Cause_Line_BG_CableField.COLUMN_CAUSE_LINE_BG_CABLE_ID)));

			curItem.setCat_Cause_Constr_Unqualify_Id(cursor.getLong(cursor
					.getColumnIndex(Cause_Line_BG_CableField.COLUMN_CAT_CAUSE_CONSTR_UNQUALIFY_ID)));

			curItem.setSync_Status(Integer.parseInt(cursor.getString(cursor
