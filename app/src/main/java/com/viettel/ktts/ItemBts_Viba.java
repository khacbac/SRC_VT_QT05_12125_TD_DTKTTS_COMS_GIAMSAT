...
public class ItemBts_Viba extends RelativeLayout implements OnClickListener {
	...
	public ItemBts_Viba(Context context) {
		super(context);
	...
	public ItemBts_Viba(Context context, View rowRoot) {
		super(context);
	...
	public void initRow(View rowRoot) {
		tv_stt = (TextView) rowRoot
				.findViewById(R.id.tv_supervision_bts_viba_stt);
		tv_tenhangmuc = (TextView) rowRoot
				.findViewById(R.id.edt_supervision_bts_viba_tenhangmuc);
		rd_dat = (CheckBox) rowRoot
				.findViewById(R.id.rdo_supervision_bts_viba_dat);
		rd_khongdat = (CheckBox) rowRoot
				.findViewById(R.id.rdo_supervision_bts_viba_khongdat);

		rd_dat.setOnClickListener(this);
		rd_khongdat.setOnClickListener(this);

		tv_nnkd = (TextView) rowRoot
				.findViewById(R.id.edt_supervision_bts_viba_nn_khongdat);
		tv_nnkd.setOnClickListener(this);
		tv_diengiai = (TextView) rowRoot
				.findViewById(R.id.edt_supervision_bts_viba_diengiai);
	...
	public void setData(Cause_Bts_Cat_WorkEntity cause_bts_catWork) {
		...
		if (cause_bts_catWork.getBts_Cat_WorkEntity().getStatus() == Constants.TANK_STATUS.DAT) {
			...
			if (countNnkdCheck > 0) {
				// if (cause_bts_catWork.getListCauseUniQualify().size() > 0) {
				tv_nnkd.setText(StringUtil.getString(R.string.text_view_dot));
			} else {
				tv_nnkd.setText(StringUtil.getString(R.string.text_empty));
			}
	...
	public void onClick(View arg0) {
		...
		switch (arg0.getId()) {
		case R.id.edt_supervision_bts_viba_nn_khongdat:
			cause_bts_catWork.setiField(Constants.BTS_VIBA_EDIT.UNQUALIFY);
			onEvent.onEvent(OnEventControlListener.EVENT_SUPERVISION_UNQUALIFY,
					null, cause_bts_catWork);
			break;
		// click dien giai
		case R.id.edt_supervision_bts_viba_diengiai:
			cause_bts_catWork.setiField(Constants.BTS_VIBA_EDIT.FAIL_DESC);
			onEvent.onEvent(OnEventControlListener.EVENT_SUPERVISION_EDIT,
					null, cause_bts_catWork);
			break;
		case R.id.rdo_supervision_bts_viba_dat:
			if (rd_dat.isChecked()) {
			...
			onEvent.onEvent(OnEventControlListener.EVENT_CHOICE_ACCESS_DAT,
					null, cause_bts_catWork);
			break;
		case R.id.rdo_supervision_bts_viba_khongdat:
			if (rd_khongdat.isChecked()) {
				cause_bts_catWork.getBts_Cat_WorkEntity().setStatus(
