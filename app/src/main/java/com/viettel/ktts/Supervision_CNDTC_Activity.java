...
public class Supervision_CNDTC_Activity extends SupervisionBtsBaseActivity {
	...
	private Button save;
	private CheckBox rdBuilder;
	private CheckBox rdAssembly;
	private CheckBox rdEquiment;
	...
	private List<CatConstructProgressEntity> itemCatConstruct = new ArrayList<CatConstructProgressEntity>();
	private Supervision_CNDTCController supervisionCNDTCCtroller;
	private VNMEditTextClearable txt_builder_number_employees;
	private VNMEditTextClearable txt_assembly_number_employees;
	private VNMEditTextClearable txt_equiment_number_employees;
	//---
	...
	public Constr_Construction_EmployeeEntity getConstr_Construction_Employee() {
		return (Constr_Construction_EmployeeEntity) getIntent().getExtras().getSerializable(IntentConstants.INTENT_DATA);
	}

	public void initComponent() {
		spvBts_KLView = addView(R.layout.supervision_bts_cndtc_activity,R.id.rl_login);
		// textview
		...
		tvDateUpdate = (TextView) spvBts_KLView.findViewById(R.id.tv_bts_cndtc_date_update);
		// combobox
		...
		// radiobutton
		rdBuilder = (CheckBox) spvBts_KLView.findViewById(R.id.rd_bts_cndtc_group_builder);
		rdAssembly = (CheckBox) spvBts_KLView.findViewById(R.id.rd_bts_cndtc_group_assembly);
		rdEquiment = (CheckBox) spvBts_KLView.findViewById(R.id.rd_bts_cndtc_group_equiment);

		rdBuilder.setOnClickListener(this);
		rdAssembly.setOnClickListener(this);
		rdEquiment.setOnClickListener(this);
		...
		save = (Button) spvBts_KLView.findViewById(R.id.btn_bts_cndtc_save);
		save.setOnClickListener(this);
		//KienPV add new 12/09/2016
		this.txt_builder_number_employees = (VNMEditTextClearable)spvBts_KLView.findViewById(R.id.txt_builder_number_employees);
		this.txt_assembly_number_employees = (VNMEditTextClearable)spvBts_KLView.findViewById(R.id.txt_assembly_number_employees);
		this.txt_equiment_number_employees = (VNMEditTextClearable)spvBts_KLView.findViewById(R.id.txt_equiment_number_employees);
		//---
	...
	public void initData() {
		...
		supervision_bts_kl_tv_thietke.setText(GloablUtils.getStringBTSInfo(isInfomation));
		
		supervisionConstroller = new Supervision_ConstrController(this);
		supervisionCNDTCCtroller = new Supervision_CNDTCController(this);
		try{
			itemCatConstruct =  (List<CatConstructProgressEntity>) supervisionCNDTCCtroller.getItemCatConstruct();
		}catch(Exception ex){ex.printStackTrace();}
	}

	public void setData() {
		supervision_bts_tv_matram.setText(constr_ConstructionItem.getStationCode());
		supervision_bts_tv_mact.setText(String.valueOf(constr_ConstructionItem.getConstrCode()));
		...
		// Cause_Bts_Pillar_AntenController btsPillarAntenController = new
		// Cause_Bts_Pillar_AntenController(
		// this);
		// Cause_Bts_Cat_WorkController causeCatWorkController = new
		// Cause_Bts_Cat_WorkController(
		// this);
		// Cause_Bts_Power_PoleController btsPowerPoleController = new
		...
		// catWorkEntityLdc =
		// causeCatWorkController.getCause_Bts_Cat_WorkEntity(
		...
		// listCauseCatWorkEquipEntity = new
		// ArrayList<Cause_Bts_Cat_WorkEntity>();
		...
		// listCauseCatWorkLowVibaEntity = new
		// ArrayList<Cause_Bts_Cat_WorkEntity>();
		...
		// listCauseCatWorkHeighVibaEntity = new
		// ArrayList<Cause_Bts_Cat_WorkEntity>();
		...
		// if (constrItem.getSupervision_Status() ==
		// Constants.SUPERVISION_STATUS.DAT) {
		// rb_dat.setChecked(true);
		// } else if (constrItem.getSupervision_Status() ==
		// Constants.SUPERVISION_STATUS.CHUADAT) {
		// rb_kdat.setChecked(true);
		// }
		// }

//		 rdAssembly.setChecked(true);
//		 rdBuilder.setChecked(true);
//		 rdEquiment.setChecked(true);
		java.text.DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		String date = df.format(Calendar.getInstance().getTime());
		tvDateUpdate.setText(date);
		
		//KienPV add new 12/09/2016
		if(itemCatConstruct != null && itemCatConstruct.size() >0){
			Log.e("size: ",itemCatConstruct.size()+"");
			for(CatConstructProgressEntity item : itemCatConstruct){
				if(item.getCode().equals(Constants.XAY_DUNG)){
					 rdBuilder.setChecked(item.getIsActive() == 1);
					 this.txt_builder_number_employees.setEnabled(item.getIsActive() == 1);
					 if(item.getIsActive() == 1){this.txt_builder_number_employees.setText(String.valueOf(item.getNumber()));}
				}
				if(item.getCode().equals(Constants.LAP_DUNG)){
					rdAssembly.setChecked(item.getIsActive() == 1);
					this.txt_assembly_number_employees.setEnabled(item.getIsActive() == 1);
					if(item.getIsActive() == 1){this.txt_assembly_number_employees.setText(String.valueOf(item.getNumber()));}
				}
				if(item.getCode().equals(Constants.THIET_BI)){
					rdEquiment.setChecked(item.getIsActive() == 1);
					this.txt_equiment_number_employees.setEnabled(item.getIsActive() == 1);
					if(item.getIsActive() == 1){this.txt_equiment_number_employees.setText(String.valueOf(item.getNumber()));}
				}
			}
		}else{
			Log.e("size: ","null");
			 rdAssembly.setChecked(false);
			 rdBuilder.setChecked(false);
			 rdEquiment.setChecked(false);
		}
		// ---
	...
	public void onClick(View v) {
		switch (v.getId()) {
		// click chon nhom xay dung		
		case R.id.rd_bts_cndtc_group_builder:			
			if (rdBuilder.isChecked()) {
				rdBuilder.setChecked(true);
				this.txt_builder_number_employees.setEnabled(true);
			} else {
				rdBuilder.setChecked(false);
				this.txt_builder_number_employees.setEnabled(false);
				this.txt_builder_number_employees.setText("");
			}
			
			break;
		// click chon nhom lap dung
		case R.id.rd_bts_cndtc_group_assembly:					
			if (rdAssembly.isChecked()) {
				rdAssembly.setChecked(true);
				this.txt_assembly_number_employees.setEnabled(true);
			} else {
				rdAssembly.setChecked(false);
				this.txt_assembly_number_employees.setEnabled(false);
				this.txt_assembly_number_employees.setText("");
			}
			
			break;
		// click chon lap TB
		case R.id.rd_bts_cndtc_group_equiment:
			if (rdEquiment.isChecked()) {
				rdEquiment.setChecked(true);
				this.txt_equiment_number_employees.setEnabled(true);
			} else {
				rdEquiment.setChecked(false);
				this.txt_equiment_number_employees.setEnabled(false);
				this.txt_equiment_number_employees.setText("");
			}			
			break;

		// TODO: Cập nhật trạng thái cho toàn bộ công trình
		// click save
		case R.id.btn_bts_cndtc_save:
			...
			//
			// }
			if (!rdBuilder.isChecked() && !rdAssembly.isChecked()
					&& !rdEquiment.isChecked()) {
				Toast.makeText(
						this,
						StringUtil.getString(R.string.text_message_must_choice),
	...
	private void saveDataKL() {
		try {
			...
			Supervision_CNDTCController supervision_CNDTCController = new Supervision_CNDTCController(
					this);
			String iStatusBuilder = "";
			String iStatusAssembly = "";
			String iStatusEquiment = "";
			if (this.rdBuilder.isChecked()) {
				iStatusBuilder = Constants.SUPERVISION_CNDTC.GROUP_BUILDER;
			}
			if (this.rdAssembly.isChecked()) {
				iStatusAssembly = Constants.SUPERVISION_CNDTC.GROUP_ASSEMBLY;
			}

			if (this.rdEquiment.isChecked()) {
				iStatusEquiment = Constants.SUPERVISION_CNDTC.GROUP_EQUIMENT;
			}

			if (iStatusBuilder != "") {
				long idConstrProgress = Ktts_KeyController.getInstance()
						.getKttsNextKey(Constr_Progress_Field.TABLE_NAME);
				long idSupervion = Ktts_KeyController.getInstance()
						.getKttsNextKey(Supervision_CNDTCField.TABLE_NAME);
				if (idSupervion == 0 || idConstrProgress == 0) {
					Toast.makeText(this,
							StringUtil.getString(R.string.text_out_of_key),
							Toast.LENGTH_SHORT).show();
					return;
				}
				CatConstructEntity catConstructEntity = supervision_CNDTCController
						.getItemCatConstruct(iStatusBuilder);
				ConstrProgressEntity constrProgressEntity = new ConstrProgressEntity();
				// Kiem tra da ton tai ban ghi ung voi cong trinh va ngay hien tai chua?
				Supervision_CNDTCEntity supervision_CNDTCEntity = new Supervision_CNDTCEntity();
				if(supervision_CNDTCController.checkItemByDate(DateConvert.convertStringToDate(tvDateUpdate.getText()
						.toString().trim()),String.valueOf(constr_ConstructionItem.getConstructId()))){
					supervision_CNDTCEntity = supervision_CNDTCController.getCNDTCItem(DateConvert.convertStringToDate(tvDateUpdate.getText()
						.toString().trim()),String.valueOf(constr_ConstructionItem.getConstructId()));
					
					supervision_CNDTCEntity.setReportDate(DateConvert.convertStringToDate(tvDateUpdate.getText()
							.toString().trim()));

					supervision_CNDTCEntity.setSyncStatus(Constants.SYNC_STATUS.EDIT);

					supervision_CNDTCEntity.setProcessId(0);
					// insert vao bang constr_progress_construct

					supervision_CNDTCController.updateConstrProgressConstruct(supervision_CNDTCEntity);
					constrProgressEntity = supervision_CNDTCController.getConstrProgressItem(supervision_CNDTCEntity.getSupervisionProgressContructId());
					// insert tiep vao bang trung gian
					
					constrProgressEntity.setCatConstructId(catConstructEntity.getCatConstructId());
					constrProgressEntity.setEmployeeId(GlobalInfo.getInstance()
							.getUserId());
					constrProgressEntity
							.setSupervisionConstrId(constr_ConstructionItem
									.getSupervision_Constr_Id());
					constrProgressEntity.setSyncStatus(Constants.SYNC_STATUS.EDIT);
					constrProgressEntity.setProcessId(0);
					constrProgressEntity.setConstrProgressNumberEmployees(Integer.parseInt(this.txt_builder_number_employees.getText().toString().trim().isEmpty() ? "0" : this.txt_builder_number_employees.getText().toString().trim()));
					
					supervision_CNDTCController.updateConstrProgress(constrProgressEntity);
				}else{
					supervision_CNDTCEntity.setReportDate(DateConvert.convertStringToDate(tvDateUpdate.getText()
							.toString().trim()));
					// insert cac ban ghi tuong ung duoc chon
					supervision_CNDTCEntity.setConstructId(constr_ConstructionItem
							.getConstructId());
					supervision_CNDTCEntity.setEmployeeId(GlobalInfo.getInstance()
							.getUserId());
					supervision_CNDTCEntity
							.setSupervisionConstrId(constr_ConstructionItem
									.getSupervision_Constr_Id());
					supervision_CNDTCEntity.setSyncStatus(Constants.SYNC_STATUS.ADD);

					supervision_CNDTCEntity
							.setSupervisionProgressContructId(idSupervion);
					supervision_CNDTCEntity.setProcessId(0);
					// insert vao bang constr_progress_construct

					supervision_CNDTCController.addItem(supervision_CNDTCEntity);
					// insert tiep vao bang trung gian
					
					constrProgressEntity.setCatConstructId(catConstructEntity.getCatConstructId());
					constrProgressEntity.setConstrProgressConstructId(idSupervion);
					constrProgressEntity.setConstrProgressId(idConstrProgress);
					constrProgressEntity.setEmployeeId(GlobalInfo.getInstance()
							.getUserId());
					constrProgressEntity
							.setSupervisionConstrId(constr_ConstructionItem
									.getSupervision_Constr_Id());
					constrProgressEntity.setSyncStatus(Constants.SYNC_STATUS.ADD);
					constrProgressEntity.setProcessId(0);
					constrProgressEntity.setConstrProgressNumberEmployees(Integer.parseInt(this.txt_builder_number_employees.getText().toString().trim().isEmpty() ? "0" : this.txt_builder_number_employees.getText().toString().trim()));
					
					supervision_CNDTCController.addItemConstrProcess(constrProgressEntity);
				}
			...
			if (iStatusAssembly != "") {
				...
				if(supervision_CNDTCController.checkItemByDate(DateConvert.convertStringToDate(tvDateUpdate.getText()
					...
					constrProgressEntity.setSyncStatus(Constants.SYNC_STATUS.EDIT);
					constrProgressEntity.setProcessId(0);
					constrProgressEntity.setConstrProgressNumberEmployees(Integer.parseInt(this.txt_assembly_number_employees.getText().toString().trim().isEmpty() ? "0" : this.txt_assembly_number_employees.getText().toString().trim()));
			...
			if (iStatusEquiment != "") {
				...
				if(supervision_CNDTCController.checkItemByDate(DateConvert.convertStringToDate(tvDateUpdate.getText()
					...
					constrProgressEntity.setSyncStatus(Constants.SYNC_STATUS.EDIT);
					constrProgressEntity.setProcessId(0);
					constrProgressEntity.setConstrProgressNumberEmployees(Integer.parseInt(this.txt_equiment_number_employees.getText().toString().trim().isEmpty() ? "0" : this.txt_equiment_number_employees.getText().toString().trim()));
	...
	private void setHeader() {
		final Header myActionBar = (Header) spvBts_KLView
		...
		myActionBar.addAction(new Header.Action() {
			@Override
			public void performAction(View view) {
				requestSync(Supervision_CNDTC_Activity.this);
			}
