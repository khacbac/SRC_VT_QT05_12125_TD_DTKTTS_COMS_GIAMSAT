...
public class MemoryUtils {
	...
	public File initCacheRoot() {
		...
		Mylog.e("PhucNT4", "dung luong external available "
				+ memAvailExternal);
		float memAvailInternal = megabytesAvailable(new File(Environment
				.getDataDirectory().getPath()));
		Mylog.e("PhucNT4", "dung luong internal available "
				+ memAvailInternal);
		...
		if (cacheRoot != null)
			Mylog.e("PhucNT4", "thu muc cache root " + cacheRoot.getPath());
	...
	private File getImageCachePath(int typeCache) {
		...
		if (cache == null)
			return null;
		String server_revision = GlobalInfo.getInstance().getProfile().getServerRevision();
	...
	public long getFileSize(File folder) {
		...
		if (folder == null)
			return 0;
		Mylog.e("PhucNT4", "Folder: " + folder.getName());
		long foldersize = 0;
